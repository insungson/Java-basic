package day09chat;	//?��?��?��?? ?��?��값을 ?��?���? 줬는�?�? ?��각해보면 ?��?���? ?��?��?��?���? ?��?���? ?�� 것이?��.
...
public class ChatServerProcessThread extends Thread {
	private static final String PROTOCOL_DIVIDER = ":"; 	//?��로토?�� 구분?���? ?��?��?�� ??..
	
	private String nickname;
	private Socket socket;
	private List<PrintWriter> listPrintWriters;
	
	public ChatServerProcessThread( Socket socket, List<PrintWriter> listPrintWriters ) {	//?��?��?��?��?�� 받�? 각각 ?��?��?���? ?��린다.
		this.socket = socket;
	...
	public void run() {
		...
		try {
			//스트림 얻기  //  지금 연결된 현재의 클라이언트와의 통신을 위한 쓰기 읽기 객체 생성
			bufferedReader = new BufferedReader( new InputStreamReader( socket.getInputStream(), StandardCharsets.UTF_8 ) );
			printWriter = new PrintWriter( new OutputStreamWriter( socket.getOutputStream(), StandardCharsets.UTF_8 ), true );
			
			//리모트 호스트 정보 얻기
			InetSocketAddress inetSocketAddress = (InetSocketAddress)socket.getRemoteSocketAddress();	//remote?��?�� 
			...
			while( true ) {
				...
				if( request == null ) {
					...
					doQuit( printWriter ); //기존?�� printWriter�? 리스?��?��?�� ?��거한?��. ?�� ?��?��?�� ?��?���??�� ?��?��?��보자
					break;
				}
				...
				*/
				if( "join".equals( tokens[0] ) ) {
					doJoin( printWriter, tokens[1] );	//printWriter ?? tokens[1] ?���?(?��?��?�� ?��?��?��?�� ?��?��?��)?�� ?��?��간다.
